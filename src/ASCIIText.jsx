...
class CanvasTxt {
  ...
  wrapLines() {
    ...
    if (this.txt.length <= 16 || words.length < 3) {
      return [this.txt];
    }
  ...
  resize() {
    if (this.textureRenderer) {
      this.canvas.width = Math.max(2, this.textureWidth);
      this.canvas.height = Math.max(2, this.textureHeight);
      return;
    }
    ...
    const widths = this.lines.map((line) => this.context.measureText(line).width);
    const textWidth = Math.ceil(Math.max(...widths, 0)) + 20;
    const textHeight = Math.ceil(this.lineHeight * this.lines.length) + 20;

    this.canvas.width = Math.max(2, textWidth);
    this.canvas.height = Math.max(2, textHeight);
  }
  ...
  render() {
    ...
    if (this.textureRenderer) {
      this.textureRenderer(this.context, this.canvas, {
        text: this.txt,
        color: this.color,
    ...
    this.lines.forEach((line, i) => {
      this.context.fillText(line, 10, 10 + i * this.lineHeight);
    });
  }
...
class CanvAscii {
  ...
  animate() {
    ...
    const animateFrame = (frameTime) => {
      ...
      if (document.hidden || frameTime - lastFrameTime < frameInterval) return;
  ...
  render() {
    if (!this.mesh || !this.filter || !this.textCanvas) return;
...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    setup().catch((error) => {
      console.error('ASCIIText failed to start', error);
    });
